 ...
 *
 */
public class SimpleInputWindowDocumentLink extends A implements EventListener<Event> {
	/**
	 *
	 */
	...
	public SimpleInputWindowDocumentLink(String label, int adTableId, int recordId){
		this(label, adTableId, recordId, null);
	}

	public SimpleInputWindowDocumentLink(String label, int adTableId, int recordId, EventListener<Event> clickHandle){
		super();
	...
	public void onEvent(Event event) throws Exception {
		if(event.getTarget() instanceof SimpleInputWindowDocumentLink){
			SimpleInputWindowDocumentLink documentLink = (SimpleInputWindowDocumentLink)event.getTarget();
			if (documentLink.getAdTableId() > 0 && documentLink.getAdTableId() > 0)
			{
				AEnv.zoom(documentLink.getAdTableId(), documentLink.getRecordId());

				//SimpleInputWindowMessagebox.detach
				documentLink.getParent().getParent().getParent().getParent().getParent().getParent().getParent().detach();
